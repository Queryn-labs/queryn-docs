...
Просмотрщик требует выделения типизированной границы чтения материалов поверх
этих сервисов. Core возвращает `FileTypeDescriptor`, а main формирует
`MaterialDescriptor` с относительным путём, версией файла и разрешёнными
действиями. Большие бинарные данные выдаются через ограниченный capability
handle. Renderer не должен получать абсолютный путь или читать большой файл
целиком. Целевая модель описана в
[архитектуре просмотрщика](./material-viewers.md).
...
Ошибки IPC передаются как проверенный структурированный конверт. Renderer
локализует известный код и выбирает действие восстановления, но не разбирает
техническую строку. Журналы и диагностический экспорт остаются в main и runtime.
Сквозной контракт описан в
[ошибках, журналах и диагностике](./errors-and-diagnostics.md).

...
- `preload`: типизированный узкий мост `window.osnova`. Renderer не получает
...
- `runtime-service`: запускает дочерний процесс `osnova-runtime serve`, создаёт
...
- `project-service`: адаптер над `@osnova/project` для создания, открытия,
...
операции, модели, контекст и агент принадлежат `osnova-runtime`, а desktop
...
`Queryn Intelligence`. `Osnova` остаётся названием продукта, формата проекта,
...
- рабочая область без открытого проекта: `workspace-home`,
  `WorkspaceLayoutRenderer` и системный виджет `osnova.projects`;
...
[`osnova-desktop/src/preload/index.ts`](https://github.com/Queryn-labs/osnova-desktop/blob/main/src/preload/index.ts).

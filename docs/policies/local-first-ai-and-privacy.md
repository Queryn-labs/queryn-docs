...
Локальные и облачные поставщики реализуют один контракт. Встроенный каталог
содержит только подключения через OpenAI-совместимый транспорт. Получатель
фиксируется выбранным шаблоном и проверяется runtime. Для локального получателя
разрешены только адреса обратной петли, для облачного требуется внешний
HTTPS-адрес. Другие транспорты требуют отдельного адаптера.
...
Ключи доступа хранятся в macOS Keychain или Windows DPAPI. Они не записываются
в `queryn.json`, сессии или пакет расширения. При удалении поставщика его ключ
удаляется только при отсутствии других конфигураций с той же учётной записью.

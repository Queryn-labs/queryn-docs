...
- Queryn Tool Protocol: `1`.
- Local JSON-RPC: `queryn-rpc/1`.
...
Lock можно удалить и пересобрать. Permissions и сохранённые правила риска не принимаются из `.queryn`: доверие хранится локально в runtime и после переноса проекта выдаётся заново. Session events сохраняют переносимый аудит решения, но не дают проекту полномочий. Неизвестные artifact types сохраняются. Отсутствующее расширение влияет на соответствующие возможности, но не блокирует открытие проекта.
